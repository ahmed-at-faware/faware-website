import os
import re

html_path = "/Users/muhammadahmed/.gemini/antigravity-ide/brain/d047440a-1ef2-4cb4-b39c-413141b5fe4b/scratch/trypost.html"

with open(html_path, 'r', encoding='utf-8') as f:
    content = f.read()

def camel_case(s):
    if s.startswith('--'):
        return s
    parts = s.split('-')
    return parts[0] + ''.join(x.title() for x in parts[1:])

def html_to_jsx(html_str):
    html_str = html_str.replace('class=', 'className=')
    html_str = html_str.replace('for=', 'htmlFor=')
    html_str = html_str.replace('crossorigin', 'crossOrigin="anonymous"')
    html_str = html_str.replace('stroke-width=', 'strokeWidth=')
    html_str = html_str.replace('stroke-linecap=', 'strokeLinecap=')
    html_str = html_str.replace('stroke-linejoin=', 'strokeLinejoin=')
    html_str = html_str.replace('stroke-dasharray=', 'strokeDasharray=')
    html_str = html_str.replace('stroke-dashoffset=', 'strokeDashoffset=')
    html_str = html_str.replace('clip-rule=', 'clipRule=')
    html_str = html_str.replace('fill-rule=', 'fillRule=')
    html_str = html_str.replace('tabindex=', 'tabIndex=')
    html_str = html_str.replace('srcset=', 'srcSet=')
    html_str = html_str.replace('playsinline', 'playsInline')
    html_str = html_str.replace('autoplay', 'autoPlay')
    
    html_str = re.sub(r'<img([^>]*?)(?<!/)>', r'<img\1 />', html_str)
    html_str = re.sub(r'<br([^>]*?)(?<!/)>', r'<br\1 />', html_str)
    html_str = re.sub(r'<input([^>]*?)(?<!/)>', r'<input\1 />', html_str)
    html_str = re.sub(r'<hr([^>]*?)(?<!/)>', r'<hr\1 />', html_str)
    
    # Fix boolean attributes
    html_str = re.sub(r'\bdisabled\b(?!=|>|\s)', 'disabled={true}', html_str)
    html_str = re.sub(r'\bchecked\b(?!=|>|\s)', 'checked={true}', html_str)
    html_str = re.sub(r'\bdata-sonner-toaster\b(?!=|>|\s)', 'data-sonner-toaster="true"', html_str)
    html_str = re.sub(r'\bdata-reka-navigation-menu\b(?!=|>|\s)', 'data-reka-navigation-menu="true"', html_str)
    html_str = re.sub(r'\bdata-menu-item\b(?!=|>|\s)', 'data-menu-item="true"', html_str)
    html_str = re.sub(r'\bdata-reka-collection-item\b(?!=|>|\s)', 'data-reka-collection-item="true"', html_str)
    html_str = re.sub(r'\bdata-navigation-menu-trigger\b(?!=|>|\s)', 'data-navigation-menu-trigger="true"', html_str)
    html_str = re.sub(r'\bdata-grace-area-trigger\b(?!=|>|\s)', 'data-grace-area-trigger="true"', html_str)
    html_str = re.sub(r'\baria-hidden\b(?!=|>|\s)', 'aria-hidden="true"', html_str)
    
    # Extract style attributes
    def style_repl(match):
        style_str = match.group(1).replace('\n', ' ').replace('\r', '')
        if not style_str.strip():
            return 'style={{}}'
        
        rules = [r.strip() for r in style_str.split(';') if r.strip()]
        react_style = []
        for r in rules:
            if ':' in r:
                k, v = r.split(':', 1)
                k = camel_case(k.strip())
                v = v.strip().replace('"', "'")
                if k.startswith('--'):
                    react_style.append(f'"{k}": "{v}"')
                else:
                    react_style.append(f'{k}: "{v}"')
        
        return 'style={{' + ', '.join(react_style) + '}}'
        
    html_str = re.sub(r'style="([^"]*)"', style_repl, html_str)
    html_str = re.sub(r"style='([^']*)'", style_repl, html_str)
    
    # Remove script tags
    html_str = re.sub(r'<script.*?</script>', '', html_str, flags=re.DOTALL)
    
    # Remove HTML comments
    html_str = re.sub(r'<!--(.*?)-->', r'{/* \1 */}', html_str, flags=re.DOTALL)
    
    return html_str

main_match = re.search(r'<body[^>]*>(.*?)</body>', content, re.DOTALL)
if main_match:
    full_app = html_to_jsx(main_match.group(1))
    app_code = "export default function App() {\n  return (\n    <div className='font-sans antialiased h-full'>\n      " + full_app + "\n    </div>\n  );\n}"
    with open('src/App.jsx', 'w', encoding='utf-8') as f:
        f.write(app_code)

styles = re.findall(r'<style.*?>(.*?)</style>', content, re.DOTALL)
with open('src/index.css', 'w', encoding='utf-8') as f:
    f.write('@tailwind base;\n@tailwind components;\n@tailwind utilities;\n\n')
    f.write('''
@layer base {
  :root {
    --background: 0 0% 100%;
    --foreground: 240 10% 3.9%;
    --card: 0 0% 100%;
    --card-foreground: 240 10% 3.9%;
    --popover: 0 0% 100%;
    --popover-foreground: 240 10% 3.9%;
    --primary: 240 5.9% 10%;
    --primary-foreground: 0 0% 98%;
    --secondary: 240 4.8% 95.9%;
    --secondary-foreground: 240 5.9% 10%;
    --muted: 240 4.8% 95.9%;
    --muted-foreground: 240 3.8% 46.1%;
    --accent: 240 4.8% 95.9%;
    --accent-foreground: 240 5.9% 10%;
    --destructive: 0 84.2% 60.2%;
    --destructive-foreground: 0 0% 98%;
    --border: 240 5.9% 90%;
    --input: 240 5.9% 90%;
    --ring: 240 10% 3.9%;
    --radius: 0.5rem;
  }
}
''')
    for style in styles:
        f.write(style + '\n')
